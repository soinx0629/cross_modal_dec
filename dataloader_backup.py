...
def load_textgrid(textgrid_file):
    """Load a TextGrid file and return a list of intervals"""
    tg = textgrids.TextGrid(textgrid_file)
    intervals = []
    for ints in tg['words']:
        intervals.append([ints.text, ints.xmin, ints.xmax])
    return intervals

def time_to_tr(intervals, tr=2.0):
    """Convert stimulus time to TR"""
    for ints in intervals:
        if ints[0] in ['sil', 'sp']:
            continue
        ints[1] = np.ceil(ints[1] / tr)
        ints[2] = np.ceil(ints[2] / tr)
    return intervals
